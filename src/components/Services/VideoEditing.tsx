...
// Load videos for the carousel
import video1 from '../../assets/Video1.mp4';

const sliderItems: (string | null)[] = [video1, null, null, null, null];
const sliderContent = [...sliderItems, ...sliderItems, ...sliderItems];
...
const VideoEditing: React.FC = () => {
  ...
  const handleNextSlide = () => {
    setSliderIndex((prev) => (prev + 1 >= sliderContent.length - itemsToShow + 1 ? 0 : prev + 1));
  };
  const handlePrevSlide = () => {
    setSliderIndex((prev) => (prev === 0 ? sliderContent.length - itemsToShow : prev - 1));
  };

  // No auto-scroll for video editing page
    ...
    <div className="bg-[#050505] min-h-screen text-white font-sans overflow-x-hidden relative">
      ...
      <section className="relative w-full min-h-[90vh] pt-48 pb-20 px-6 flex flex-col items-center justify-center text-center z-10">
        ...
        >
          ...
          <span className="text-sm font-medium tracking-wide uppercase text-white/80">Creative Studio</span>
        </motion.div>
        ...
        >
          <h1 className="text-[14vw] sm:text-[100px] md:text-[140px] lg:text-[180px] font-black leading-[0.85] tracking-tighter italic uppercase text-white relative">
            <span className="block relative z-10 hover:text-transparent hover:-webkit-text-stroke-[2px] hover:-webkit-text-stroke-white transition-all duration-500">Video</span>
        ...
        >
          Cinematic storytelling that captivates. We craft stunning videos that 
          elevate your brand and leave a lasting impression on your audience.
        </motion.p>

        {/* Hero Interactive Button */}
        <motion.a
          href="https://wa.me/923134409152?text=Hello%20Minarix!%20I%20would%20like%20to%20start%20a%20video%20editing%20project."
          target="_blank"
                ...
                >
                  {src ? (
                    ...
                    <div className="w-full h-full bg-white/5 flex items-center justify-center rounded-md">
                      <span className="text-white/20 text-sm italic">Coming Soon</span>
                    </div>
                  )}
                </div>
              ))}
